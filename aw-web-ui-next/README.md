...
- For production, serve `dist/` under `/next` alongside the existing UI. The preferred location is aw-server's static assets folder (e.g., `aw_server/static/next`). When submodules are available, add a small router entry in aw-server to map `/next` to these assets without disturbing `/`.
