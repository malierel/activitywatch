...
const ActivityPage = () => {
  const { data: hosts = [], isLoading: isLoadingHosts, error: hostError } = useHosts();
  const { data: buckets = [], isLoading: isLoadingBuckets, error: bucketError } = useBuckets();
  ...
  return (
    <div className="space-y-6">
      ...
      {bucketError || hostError ? (
        <InlineState
          state="error"
          description="The UI could not read buckets from aw-server. Verify aw-server is running on localhost and that watcher data exists."
          technicalDetails={formatApiError((bucketError ?? hostError) as any)}
          actionLabel="Open buckets"
          onAction={() => window.open('/#/buckets', '_blank')}
        />
      ) : isLoadingBuckets || isLoadingHosts ? (
        <InlineState state="loading" />
      ) : buckets.length === 0 ? (
        <InlineState
          state="empty"
          title="No watcher buckets found for this host"
          description="Start a watcher like aw-watcher-window or aw-watcher-afk to generate activity data."
          actionLabel="Open buckets"
          onAction={() => window.open('/#/buckets', '_blank')}
        />
      ) : (
